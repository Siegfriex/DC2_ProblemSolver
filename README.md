...
이 노트북의 출력은 아래 조합에서 전 셀 재실행으로 확인했다 (2026-09-19).
...
1. `git checkout main && git checkout -b <날짜>_<주제>`
2. `.gitignore` 의 화이트리스트에 새 노트북 파일명 한 줄 추가
   (`.gitignore` 는 `*` 로 전부 막고 필요한 파일만 `!` 로 되살리는 방식이다 —
    다른 과제 노트북이 실수로 커밋되는 것을 구조적으로 막는다)
